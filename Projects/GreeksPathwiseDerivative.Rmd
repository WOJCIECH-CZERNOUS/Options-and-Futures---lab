...
# generowanie S_T
call_put_mc = function() {
  Z = rnorm(1)
  WT = sqrt(T) * Z
  S_T = S_0*exp((r - 0.5*sigma^2)*T + sigma*WT)
  return(S_T)
}
set.seed(1)
results = call_put_mc()
...
#rozniczka dY/dS_T
dY_dS_T1 = function(S_T) {
  return ( exp(-r*T) * indicator(S_T)  )
}
dS_t_dS_01 = function(S_t) {
  return (S_t / S_0)
}
# ROZNICZKA DELTA
delta = function(S_T) {
  dD = dY_dS_T1(S_T) * dS_t_dS_01(S_T)
  return (dD)
}
results = delta(S_T)
print(results)
#####################
#rozniczka dY/dS_T
dY_dS_T2 = function(S_T) {
  return ( exp(-r*T) * indicator(S_T)  )
}
dS_t_dS_02 = function(S_t) {
  return ((S_t/sigma)*(log((S_t/S_0))-(r-sigma^2/2)*T))
}
# ROZNICZKA VEGA
vega = function(S_T) {
  dv = dY_dS_T2(S_T) * dS_t_dS_02(S_T)
  return (dv)
}
results = vega(S_T)
print(results)
#######################
#rozniczka dY/dS_T
dY_dS_T3 = function(S_T) {
  return ( exp(-r*T) * indicator(S_T)  )
}
dS_t_dS_03 = function(S_t) {
  return (K*T)
}
# ROZNICZKA RHO
rho = function(S_T) {
  dr = dY_dS_T3(S_T) * dS_t_dS_03(S_T)
  return (dr)
}
results = rho(S_T)
print(results)
######################
#rozniczka dY/dS_T
dY_dS_T4 = function(S_T) {
  return ( exp(-r*T) * indicator(S_T)  )
}
dS_t_dS_04 = function(S_t) {
  return (r-S_t/2*T*(log(S_t/S_0)+(r-sigma^2/2)*T + sigma*sqrt(T)))
}
# ROZNICZKA THETA
theta = function(S_T) {
  dt = dY_dS_T4(S_T) * dS_t_dS_04(S_T)
  return (dt)
}
results = theta(S_T)
print(results)
...
```{r}
# generowanie S_T dla opcji azjatyckich
...
delta2 = function(S) {
  ...
  factor2 = c()
  for (i in 1:n){
    factor2[i] = dS_t_dS_01(S[i])
  }
  dD = dY_dS_T1(barS) * mean(factor2)
  return (dD)
}
print(mean(replicate(1000, delta2(sample_trajectory_of_S()))))
...
vega2 = function(S) {
  ...
  factor3 = c()
  for (i in 1:n){
    factor3[i] = dS_t_dS_02(S[i])
  }
  dv = dY_dS_T2(barS) * mean(factor3)
  return (dv)
}
print(mean(replicate(1000, vega2(sample_trajectory_of_S()))))
...
rho2 = function(S) {
  ...
  factor4 = c()
  for (i in 1:n){
    factor4[i] = dS_t_dS_03(S[i])
  }
  dv = dY_dS_T3(barS) * mean(factor4)
  return (dv)
}
print(mean(replicate(1000, rho2(sample_trajectory_of_S()))))
...
theta2 = function(S) {
  ...
  factor5 = c()
  for (i in 1:n){
    factor5[i] = dS_t_dS_04(S[i])
  }
  dv = dY_dS_T4(barS) * mean(factor5)
  return (dv)
}
print(mean(replicate(1000, theta2(sample_trajectory_of_S()))))
