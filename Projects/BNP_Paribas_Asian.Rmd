...
A=(m+(s^2)-log(k))/s
B=(log(k)-m)/(s)
return(exp(m+(s^2)/2)*(pnorm(A)/(1-pnorm(B))))
}
...
return((expected_LogNormal_greater(m,s,k))-(k*(1-p_LogNormal(k,m,s))))
}

put_option_on_LogNormal = function(m,s,k) { # E(K-X)_+  (używamy 'put-call parity')
return((k*(1-p_LogNormal(k,m,s)))-(expected_LogNormal_greater(m,s,k)))
}
...
payoff_Asian_conditional_call = function(trajectory) {
 barS = mean( trajectory[trajectory>b] )
return ( max(0, barS-K) )
}
...
Otrzymane wyniki, dla różnych wartości sigma oraz m,n, podajemy poniżej, wraz z czasem obliczeń. Obliczenia dokonona zostały na komputerze ... Załączamy też, dla porównania, wyniki dostępne w literaturze [2] (Table 2).
