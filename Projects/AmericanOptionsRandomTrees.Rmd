...
v_low_estimator = function(i,x) {
  ...
  if (i == m) {
    ...
    z = exp(-r*dt) * replicate(b, v_low_estimator(i+1,sample_next(x)))
    hat_v_ik = c()
    for (k in 1:b) {
      if(mean(z[-k])<payoff){
        hat_v_ik[k] = payoff
...
```{r}
q = replicate(100,v_low_estimator(0,S_0))
print(mean(q))
print(sd(q))
```
...
a każde z tych zdarzeń ma prawdopodobieństwo nie przekraczające $\delta/2$ (dla dużych $n$); ich alternatywa ma prawdopodobieństwo nie większe, niż $\delta$. Stąd $V_0(X_0)$ zawiera się w przedziale $I_\delta(n,b)$ z prawdopodobieństwem (dla dużych $n$) przynajmniej $1-\delta$.
...
V_high_estimator_faster = function(i,x) {
  ...
  if (i == m) {
    ...
    C = V_high_estimator(i+1,sample_next(x))
    return (max(payoff, exp(-r*dt) * mean(C)))
  }
}
print(system.time(V_high_estimator_faster(0,S_0)))
...
v_low_estimator_faster = function(i,x) {
  ...
  if (i == m) {
    ...
    z = exp(-r*dt) * (v_low_estimator(i+1,sample_next(x)))
    hat_v_ik = c()
     {
      if(mean(z)<payoff){
        hat_v_ik = payoff
...
print(system.time(v_low_estimator_faster(0,S_0)))
```
...
X_high_estimator_faster_European = function(i,x) {
  ...
  x=c();
for(i in 1:1000){
  x[i]=V_high_estimator(0,S_0)
}
  return(barx_V=mean(x))
}
print(barx_V)
X_low_estimator_faster_European = function(i,x) {
    x=c();
for(i in 1:1000){
  x[i]=V_low_estimator(0,S_0)
}
  barx_v=mean(x)
}
print(barx_V)
...
[3] Kalkulator online dla wyznaczania cen opcji: <http://www.calkoo.com/pl/parytet-put-call>
